 ...
 * 채로 조용히 나갔다. 지금은 자리 수 불변식이 그 자리를 메운다 — 다만 메워지는
 * 것은 마피아 자리이지 언제나 Role.MAFIA 카드는 아니다. mafiaFiller는 배타에
 * 걸려 마피아를 못 쓰면 이미 뽑힌 밀담 직업을 한 번 더 쓰므로, 마피아와 사기꾼을
 * 배타로 묶은 아래 첫 스펙에서는 사기꾼이 뽑힌 판의 메움패가 사기꾼이 된다
 * (11·12인 200시드 중 100판이 「짐승인간+사기꾼+사기꾼」, 나머지 100판이
 * 「짐승인간+마피아+마피아」). 어느 직업이 들어오는지는 배타 관계가 정하고,
 * 같은 블록의 「메운 뒤에도 마피아와 사기꾼이 한 덱에 있지 않다」가 정확히 그
 * 구별에 기댄다.
 *
