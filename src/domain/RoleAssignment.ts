 ...
 * 직업을 한 번 더 쓴다. 그쪽은 새로 어길 수 있는 규칙이 없다 — 이미 덱에
 * 있으므로 그 직업의 그룹 동료는 이 시점에 전부 빠져 있고(withoutRivals는
 * taken에 든 직업을 막지 않는다), 뽑힐 때 인원 하한을 이미 통과했다.
 *
...
export function buildRoleDeck(
	 ...
	 * 마피아 자리 수를 여기서 못 박는다.
	 *
	 * 이 지점을 지날 때 roles.length는 언제나 teamSize다 — 스펙이 무엇이든,
	 * 인원이 몇이든, 시드가 무엇이든. 위 리드 검사는 이것을 보장할 수 없다.
	 * 크기 검사 뒤에 좁아지는 뽑기가 있는 한 검사는 필요조건까지만 말하고,
	 * 실제로 몇 개가 나오는지는 뽑아 봐야 안다.
	 *
