...
export const STANDARD_RULES: RuleSet = {
	...
	deck: {
		...
		//   drawExclusive에 0이 넘어가고 이 풀에서 아무도 뽑히지 않는다. 값 7은
		//   하한이 처음 의미를 갖는 인원과 정확히 같아서 어느 방향으로도 구속하지
		//   않는다(지워도, 6이나 8로 바꿔도 오늘의 덱은 한 장도 안 바뀐다).
		//   그래도 적어 두는 것은 의도의 선언이자 미래 방어다: mafiaTeamSize가
		//   바뀌어 낮은 인원의 자리가 둘이 되는 날 이 줄이 실효를 갖는다.
		//   그 실효는 tests/deck.test.ts의 「인원 하한이 사기꾼을 막는다」가
		//   합성 스펙으로 미리 밟아 둔다
		// SHAMAN: 8인 구성부터 들어간다
