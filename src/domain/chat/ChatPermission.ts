...
export function accessOf(ctx: ChatContext, channel: ChatChannel): ChannelAccess {
	...
	if (channel === ChatChannel.MAFIA) {
		/*
		 * 죽으면 밀담이 닫힌다. 무덤에 남는 채널은 유령과 방 둘뿐이다.
		 *
		 * locked가 아니라 NONE인 것이 요점이다. 읽기만 열어 두어도 무덤 쪽에서
		 * 읽은 밀담은 유령 채널을 타고 영매를 거쳐 낮으로 돌아온다 — 표시의
		 * 문제가 아니라 밸런스이고, 아래 영매의 낮 분기가 NONE인 것과 같은
		 * 이유다. 탭에서 사라지므로 죽는 순간 CHANNEL_ORDER가 유령으로
		 * 옮겨 주는 것도 여기서 따라온다.
		 *
		 * mafiaChat 검사보다 **위**에 두어야 한다. 죽은 마피아는 아래 줄을
		 * 통과하기 때문이다 — 순서를 뒤집으면 규칙이 그들에게만 남고, 무덤에
		 * 앉아 오늘 밤 표적을 미리 읽는 자리가 생긴다.
		 *
		 * seated를 묻지 않는다. 방 밖에 선 사람의 alive는 LOOSE_CONTEXT의
		 * 참이라 이 줄에 걸리지 않고, 관전자는 맨 위에서 이미 돌아갔다.
		 *
		 * 잃는 것을 적어 둔다: 예전에는 죽은 사람이 밀담을 읽어 유령 채널에
		 * 옮기고 영매가 밤에 그것을 건져 왔다. 그 경로가 끊겼으므로 영매가
		 * 무덤에서 얻는 것은 성불로 확인한 직업 하나와 죽은 사람들의 추측뿐이다.
		 * 성불 능력 자체는 그대로다.
		 */
		if (!ctx.alive) return NONE;
	...
	if (channel === ChatChannel.LOVER) {
		/*
		 * 밀담과 같은 이유로 죽으면 닫는다. 여기서 새는 것은 마피아의 정체가
		 * 아니라 짝이 누구인가지만, 그것도 무덤에서 유령 채널로 옮겨지면
		 * 영매를 통해 낮으로 돌아온다.
		 *
		 * 쓰기 하나만 막는 것으로는 부족했다는 점도 같다. 한쪽이 죽으면 그날
		 * 밤 다른 쪽도 상심으로 따라 죽지만(NightPipeline의 chainLovers) 연쇄가
		 * 도는 것은 밤의 끝이라, 그 사이에 죽은 쪽이 남은 짝의 대화를 읽으며
		 * 범인을 좁힐 창이 열린다.
		 */
		if (!ctx.alive) return NONE;
