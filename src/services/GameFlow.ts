...
import { readyCount, resetRoom } from "../entities/Room.ts";
...
import { beginNight, resolveNight } from "./Night.ts";
import { finishIfDecided } from "./Outcome.ts";
import { countPlay, refreshTitle } from "./Rewards.ts";
import { clearSilhouettes, resetPlayerAppearance } from "./Stage.ts";
import { beginDay, beginVote, beginVoteResult } from "./Voting.ts";
...
function beginGame(room: Room): void {
	...
	forEachPlayer(room, (player, seat) => {
		...
		player.sendUpdated();
		openRoleCard(player, roleDef(seat.role).cardWidget).onMessage.Add(handleCardMessage);
	});
...
/**
 * 접속 중인 좌석 수.
 * seat.connected는 join/leave 이벤트에서만 갱신되므로
