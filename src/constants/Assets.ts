 ...
 * 효과음.
 *
 * 열여덟 개 전부 tools/make-sfx.py가 만든다 — 받아 온 음원이 아니다.
...
export const Sound = {
	...
	NOTE: "sfx_note.mp3",
	/**
	 * 개표가 끝나 누군가 단상에 올랐을 때.
	 *
	 * 아무도 오르지 않은 개표(동률·전원 스킵·정치인)에서는 울리지 않는다.
	 * 그 낮에는 실제로 아무 일도 일어나지 않았고, 소리는 화면이 열린 것이
	 * 아니라 사건을 알린다.
	 */
	NOMINATE: "sfx_nominate.mp3",
	/** 낮 투표로 처형됐을 때 */
	EXECUTE: "sfx_execute.mp3",
	/**
	 * 찬반투표가 부결돼 단상에 오른 사람이 살아남았을 때.
	 *
	 * EXECUTE의 짝이다. 이 소리가 없던 동안 부결은 무음이었고, "살아남았다"와
	 * "아직 결과가 안 나왔다"가 소리로 구별되지 않았다. 처형보다 한 계층 낮은
	 * 이유는 tools/make-sfx.py의 s_acquit에 적혀 있다.
	 */
	ACQUIT: "sfx_acquit.mp3",
	/** 밤사이 죽은 사람이 있을 때 */
	DEATH: "sfx_death.mp3",
...
/**
 * 깔리는 소리(BGM). 효과음과 달리 **루프로 돌고 스스로 멈추지 않는다.**
 *
 * 세 개뿐인 이유는 이 게임의 분위기가 세 가지이기 때문이다 — 밤은 조여들고,
 * 낮은 떠들썩하고, 재판은 멈춰 선다. 단계마다 곡을 붙이면 낮→투표→개표에서
 * 3초마다 곡이 바뀌는데, 그건 분위기가 아니라 소음이다. 그래서 곡은 "장면"
 * 단위로만 갈리고, 같은 곡이 이어지는 동안 다시 시작하지 않는다
 * (Screen.setScene이 room.ambience와 비교해 그것을 보장한다).
 *
 * tools/make-bgm.py가 만든다. 효과음과 같은 A 단5음계에 뿌리를 두고 있어서
 * 위에 얹히는 효과음과 음이 부딪히지 않는다. 각 8초 루프이고, 끝과 시작이
 * 같은 진폭에서 만나도록 잘라 이어붙일 때 딸깍 소리가 나지 않는다.
 */
export const Bgm = {
	/** 직업 공개와 밤 — 낮은 드론 위에 느린 심장박동 */
	NIGHT: "bgm_night.mp3",
	/** 낮 토론과 투표 — 가벼운 플럭과 셰이커 */
	DAY: "bgm_day.mp3",
	/** 최후의 반론과 찬반투표 — 초침과 조여드는 저음 */
	TRIAL: "bgm_trial.mp3",
} as const;

/**
 * BGM 공통 볼륨. 효과음보다 낮아야 한다.
 *
 * 깔리는 소리는 사건을 알리는 소리에 자리를 내줘야 한다. 둘이 같은 크기면
 * 밤에 조사음이 났는지 BGM의 한 음이 났는지 구분되지 않는다. 0.35는
 * 효과음(0.8)의 절반보다 낮아, 말소리와 발소리를 덮지 않는 선이다.
 */
...
export const WidgetSize = {
	 ...
	 *
	 * 320px·36% → 290px·33%. PHASE와 같은 이유로 줄였지만 세는 방법이 달랐다.
	 * 이 화면의 .stand는 flex:1 1 auto라 남는 자리를 그냥 먹어서, 층의 합을
	 * 손으로 세도 "얼마가 남았는가"가 나오지 않는다. panel의 height를 auto로
	 * 풀고 실제로 재면 폭에 따라 257~273px이었다(글자가 clamp(…vw…)라 폰 폭
	 * 346·377·413px에서 값이 다르다. 가장 큰 값은 지시문이 두 줄로 접히는
	 * 좁은 폭 쪽이 아니라 글자가 커지는 넓은 폭 쪽이다).
	 *
	 * 36%는 850px 폰에서 306px이라 .stand가 내용 75px을 119px로 늘려 들고
	 * 있었다 — 화면 가운데 44px이 빈 공기였고, 비어 있다는 것이 눌러 보이지도
	 * 않는 종류의 낭비다. 33%면 실제 폰(393×852·430×932)에서 19px 이상이
	 * 남아 지시문이 두 줄로 접혀도 버틴다.
	 *
	 * 남은 것: 채팅을 펼치면 MAIN_TIGHT배가 되어 20%(170px)가 되는데, 이
	 * 화면의 고정 층 합(122px + 간격 40 + 여백 26 = 188px)이 그보다 크다.
	 * 그때는 .stand가 0으로 눌려 단상에 오른 사람의 이름이 사라진다. 36%
	 * 시절에도 22%=187px로 이미 넘었으니 이번 축소가 만든 것은 아니다.
	 */
	JUDGEMENT: { width: 380, height: 290, mobile: 33 },
	/**
	 * 전원의 직업 공개 목록이 들어간다.
	 *
	 * 실측에서 유일하게 예산이 모자란 화면이다. 46%는 850px 폰에서 391px이고
	 * 목록에 돌아오는 자리는 212px인데, 필요한 자연 높이는 6명 376px · 8명
	 * 443px · 12명 577px이었다(줄 하나가 30px). 줄일 빈 자리가 있는 게 아니라
	 * 오히려 8명 판에서 두 줄, 12명 판에서 여섯 줄이 잘려 있었다.
	 *
	 * 46%는 메인 위젯의 천장이다(위 세로 예산). 늘리면 게임 화면이 48% 아래로
	 * 내려가고, 16초짜리 화면 하나를 위해 깨뜨릴 예산이 아니다. 그래서 %는
	 * 그대로 두고 두 가지를 했다 — 줄을 26px로 줄여 일곱 줄이 보이게 했고
	 * (6명 판은 이제 전부 들어온다), 잘린 목록이 잘린 것처럼 보이게 했다
	 * (gameOver.html의 .cut). 모바일 웹뷰의 스크롤바는 손을 대는 동안만
	 * 보이므로 그 전에는 아래에 더 있다는 표시가 아무 데도 없었다.
	 *
	 * 데스크톱만 420 → 480px이다. 거기에는 예산이 없고, 이 값이 8명 판의
	 * 명단(404px)을 스크롤 없이 보여주면서 이 저장소에서 가장 큰 위젯
	 * (CARD_BOOK 480px)을 넘지 않는 높이다.
	 */
	GAME_OVER: { width: 380, height: 480, mobile: 46 },
	/**
	 * 밤/아침: 읽을 것만 있고 조작이 없다.
	 *
	 * 모바일 28 → 24. 재설계에서 이 화면의 층이 줄어들지는 않았지만
	 * (신분 줄 · 지시 줄 · 시간 선 · 무대 · 사건 목록) 층마다 고정 높이가
	 * 정해졌고 합이 203px이다. 28%는 그보다 큰 값이라 아래가 남았고,
	 * 남은 자리는 게임 화면을 가리는 것 말고는 아무 일도 하지 않는다.
	 */
	PHASE: { width: 380, height: 260, mobile: 24 },
	/**
	 * 펼친 채팅창.
	 *
